# DeployPilot Requirements

## 1. Purpose

DeployPilot is a DevOps Assistant that helps users produce practical, reviewable, and reusable outputs from a focused workflow. The product must be useful as a public repository while still being compatible with Cerebra MCP as an internal orchestration, review, testing, security, and DevOps layer.
...
- Name: DeployPilot
- Category: DevOps Assistant
- Summary: Inspect and generate deployment packages: Dockerfile, Docker Compose, GitHub Actions, environment checklist, health check, deployment README, and rollback guide.
- Priority: Phase 3 support product
- Cerebra MCP fit: CerebraDevops-mcp, CerebraSecurity-mcp, CerebraTesting-mcp, CerebraReview-mcp

## 3. Goals

- Provide a complete MVP path from input to usable output.
- Make DeployPilot understandable to public GitHub visitors without requiring private Cerebra context.
...
The person who runs DeployPilot to create or inspect the target artifact. They need clear setup, fast feedback, and usable output.
...
- Output must match the DevOps Assistant use case.
...
Main risk areas: unsafe deployment defaults, missing migrations, secret leakage, wrong health checks, rollback gaps
...
- a new user can understand DeployPilot from README.md
