...
DeployPilot is a deployment assistant for startups, solo developers, DevOps-light teams. The product must support the complete path from setup to reviewed output, with enough traceability and operational control for real use by a small team.

## 2. Goals

- Deliver one reliable end-to-end workflow for deployment assistant.
...
- environment inventory, deployment checklist, preflight checks, release notes, rollback runbook
...
- The product must provide a guided intake form for deployment assistant.
...
- The product should support at least one integration handoff in MVP: GitHub Actions, Docker, Vercel, Railway, cloud logs.
...
Primary risk areas: unsafe deploys, missing migrations, secret drift, no rollback owner.
